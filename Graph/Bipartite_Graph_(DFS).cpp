...
• Its vertices can be divided into two disjoint and independent
  sets U and V such that every edge connects a vertex in U to
  one in V.
• The graph does not contain any odd-length cycles
• The graph is 2 colorable 

Bipartite Graph = That can be coloured using 2 color such that no two adjacent nodes have same color */

/* Pseudocode
• Traverse the graph.
• If the current node is of red color, paint its neighbours as blue.
If you are able to color the graph successfully, then the graph is
bipartite. */
