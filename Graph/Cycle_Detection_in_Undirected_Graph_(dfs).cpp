...
/* Solution: Using DFS 
Any adjacent node other that parent node if visited then the graph has cycle. We can use recursion to perform DFS and keep track of the parent node for each visited node. If we encounter an adjacent node that is already visited and is not the parent of the current node, then we have found a cycle in the graph. */
